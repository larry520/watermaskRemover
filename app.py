...
async def upload_video(
    ...
    use_fast: bool = False,
    frame_by_frame: bool = False  # 新增：逐帧检测
):
    """
    上传视频并去除水印
    ...
    if frame_by_frame:
        ...
        background_tasks.add_task(
            ...
            task_id, input_path, skip_frames, quality, keep_audio
        )
    elif use_fast or manual_box or box == "":
        # 快速模式（固定 Mask）
        background_tasks.add_task(
            process_fast_video_task,
            task_id, input_path, manual_box, quality, keep_audio
        )
    else:
        # 标准模式（自适应检测）
        background_tasks.add_task(
...
def process_video_task(task_id: str, input_path: Path, skip_frames: int, quality: str, keep_audio: bool):
    """后台处理视频（标准模式）"""
    try:
        tasks[task_id]["status"] = "processing"
...
def process_fast_video_task(task_id: str, input_path: Path, manual_box, quality: str, keep_audio: bool):
    """后台处理视频（快速模式）"""
    try:
        tasks[task_id]["status"] = "processing"
...
def process_video_frame_by_frame_task(task_id: str, input_path: Path, skip_frames: int, quality: str, keep_audio: bool):
    """后台处理视频（逐帧检测模式）"""
    try:
        tasks[task_id]["status"] = "processing"
        ...
        # 使用逐帧检测（adaptive=True, skip_frames=1）
        result = processor.process_video(
            input_path, output_path,
            skip_frames=skip_frames,
            quality=quality,
            keep_audio=keep_audio,
            adaptive=True  # 启用自适应检测（逐帧）
        )
