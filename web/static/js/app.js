...
document.getElementById('videoProcessBtn').addEventListener('click', async function() {
    ...
    const formData = new FormData();
    formData.append('file', file);
    formData.append('skip_frames', document.getElementById('videoSkipFrames').value);
...
document.getElementById('fastVideoFile').addEventListener('change', function(e) {
    const file = e.target.files[0];
    if (file) {
        uploadedFiles.fastVideo = file;
        document.getElementById('fastVideoProcessBtn').disabled = false;
        showStatus('视频已上传，可以开始快速处理', 'success');
    }
});

setupDragDrop('fastVideoUpload', 'fastVideoFile');

document.getElementById('fastVideoProcessBtn').addEventListener('click', async function() {
    const file = uploadedFiles.fastVideo;
    if (!file) return;
    
    this.disabled = true;
    document.getElementById('fastVideoLoader').style.display = 'block';
    document.getElementById('fastVideoProgress').style.display = 'block';
    
    const formData = new FormData();
    formData.append('file', file);
    
    const box = document.getElementById('fastVideoBox').value.trim();
    if (box) {
        formData.append('box', box);
    }
    
    formData.append('quality', document.getElementById('fastVideoQuality').value);
    formData.append('save_mask', document.getElementById('fastVideoSaveMask').checked);
    
    try {
        updateProgress('fastVideo', 5, '上传中...');
        ...
        const result = await response.json();
        taskIds.fastVideo = result.task_id;
        
        updateProgress('fastVideo', 10, '快速处理中...');
        showStatus('使用固定 Mask 快速处理，速度提升 10-50 倍！', 'info');
        
        pollTaskStatus('fastVideo', result.task_id);
        
    } catch (error) {
        showStatus('处理失败: ' + error.message, 'error');
        this.disabled = false;
        document.getElementById('fastVideoLoader').style.display = 'none';
        document.getElementById('fastVideoProgress').style.display = 'none';
    }
});
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.type.startsWith('video/')) {
                ...
                if (videoMode === 'frame_by_frame') {
                    ...
                    formData.append('skip_frames', 1);
                } else if (videoMode === 'fast') {
                    // 快速模式
                    formData.append('use_fast', true);
                } else {
                    // 标准模式（自适应检测）
                    formData.append('skip_frames', 2);
                }
...
function downloadFastVideo() {
    const taskId = taskIds.fastVideo;
    if (taskId) {
        window.location.href = `${API_BASE}/download/${taskId}`;
    }
}
...
async function openFastVideoFile() {
    const taskId = taskIds.fastVideo;
    if (taskId) {
        try {
            const response = await fetch(`${API_BASE}/open_file/${taskId}`);
